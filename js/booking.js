/**
 * Montego Bay request-to-book form (Phase 14D).
 * Server-side pricing is authoritative — client total is for review only.
 */
(function () {
  const form = document.getElementById("mgb-booking-form");
  if (!form || !window.MGB_COMMERCIAL) return;

  const cfg = window.MGB_COMMERCIAL;
  ...
  const guestModel = product.guestModel || "adult_child_infant";
  ...
  function refresh() {
    ...
    const children = guestModel === "adult_child_infant" ? num("children", 0) : 0;
    const infants = guestModel === "adult_child_infant" ? num("infants", 0) : 0;
    ...
    const total =
      adults * adultRate +
      (childRate == null ? 0 : children * childRate) +
      (infantRate == null ? 0 : infants * infantRate);
    if (totalGuests > max) set("rev-total", "Too many guests (max " + max + ")");
    else set("rev-total", "USD $" + total);
    return { adults, children, infants, totalGuests, totalCents: Math.round(total * 100) };
  }
  ...
  function sessionId() {
    ...
    return "mgb-" + Date.now() + "-" + Math.random().toString(36).slice(2, 10);
  }
  ...
  form.addEventListener("submit", async function (event) {
    ...
    const totalCents = Math.round(
      adults * adultRate * 100 +
        (childRate == null ? 0 : children * childRate * 100) +
        (infantRate == null ? 0 : infants * infantRate * 100),
    );
    const payload = {
