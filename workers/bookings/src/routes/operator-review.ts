...
import { findMontegoBayBookingProduct } from "../../../../shared/destinations/montego-bay-products";
...
function errorPage(title: string, message: string, httpStatus = 403): Response {
  const body = `
    <div class="eyebrow">Montego Bay booking operations</div>
...
export async function handleOperatorReviewPage(request: Request, env: Env): Promise<Response> {
  ...
  const product = findMontegoBayBookingProduct(booking.product_id);
  ...
  if (booking.status === "confirmed") {
    return htmlPage(
      "Booking confirmed",
      `<div class="eyebrow">Montego Bay booking request</div><h1>Booking already confirmed</h1><div class="badge">Confirmed</div><p class="muted">Reference ${escapeHtml(booking.booking_reference)} is already confirmed. The customer confirmation email was sent when this booking was actioned.</p>${bookingSummaryHtml(booking, amountLabel, guestsLabel)}`,
    );
  }
  ...
  if (booking.status === "supplier_declined" || booking.payment_status === "refunded") {
    return htmlPage(
      "Booking declined",
      `<div class="eyebrow">Montego Bay booking request</div><h1>Booking already declined</h1><div class="badge">Declined · Refunded</div><p class="muted">Reference ${escapeHtml(booking.booking_reference)} has already been declined and refunded.</p>${bookingSummaryHtml(booking, amountLabel, guestsLabel)}`,
    );
  }
  ...
  if (step === "confirm") {
    return htmlPage(
      "Confirm booking",
      `<div class="eyebrow">Montego Bay booking request</div><h1>Confirm this excursion?</h1><div class="badge">${escapeHtml(statusBadge(booking))}</div>
  ...
  if (step === "decline") {
    ...
    return htmlPage(
      "Decline booking",
      `<div class="eyebrow">Montego Bay booking request</div><h1>Decline this booking and refund ${escapeHtml(amountLabel)}?</h1>
      ${liveWarning}
      ${bookingSummaryHtml(booking, amountLabel, guestsLabel)}
      <form class="actions" method="POST" action="/operator/review/action">
        <input type="hidden" name="ref" value="${escapeHtml(reference)}">
  ...
  return htmlPage(
    "Review booking",
    `<div class="eyebrow">Montego Bay booking request</div><h1>Review booking</h1><div class="badge">${escapeHtml(statusBadge(booking))}</div>
...
export async function handleOperatorReviewAction(request: Request, env: Env): Promise<Response> {
  ...
  if (action === "confirm") {
    return htmlPage(
      "Booking confirmed",
      `<div class="eyebrow">Montego Bay booking request</div><h1>Booking confirmed</h1><p class="muted">Reference ${escapeHtml(reference)} is now confirmed. The customer confirmation email has been queued.</p>`,
    );
  }

  return htmlPage(
    "Booking declined",
    `<div class="eyebrow">Montego Bay booking request</div><h1>Booking declined &amp; refund ${result.refunded ? "issued" : "initiated"}</h1><p class="muted">Reference ${escapeHtml(reference)} has been declined. ${result.refunded ? "The full refund succeeded." : "Stripe is processing the refund."} The customer refund email has been queued.</p>`,
  );
}
