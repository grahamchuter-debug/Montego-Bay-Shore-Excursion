 ...
 */
/** Phase 14D: live charging remains locked until Phase 14E controlled unlock. */
export const LIVE_PAYMENTS_CODE_ENABLED = false;

export const LIVE_UNLOCK_PHRASE = "MONTEGO_BAY_LIVE_UNLOCK";
...
export function liveReadinessGaps(
  ...
  if (!(env.SITE_BASE_URL || "").includes("montegobayshoreexcursion.com")) gaps.push("site_base_url");
...
export function liveCheckoutBlock(
  ...
  if (!LIVE_PAYMENTS_CODE_ENABLED) {
    return {
      ...
      message: "Live Stripe is not enabled for Montego Bay. The live code flag is off. No payment was taken.",
    };
  }
  ...
  if (!bookingsAreEnabled(env)) {
    return {
      ...
      message: "Montego Bay online booking is switched off. No payment was taken.",
    };
  }
  if (!env.DB) {
    return {
  ...
  if (!site.includes("montegobayshoreexcursion.com")) {
    return {
      code: "SITE_BASE_URL",
      message: "Live Checkout requires SITE_BASE_URL on montegobayshoreexcursion.com. No payment was taken.",
    };
  }
  if (!product) {
    return {
