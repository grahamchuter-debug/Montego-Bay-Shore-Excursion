/**
 * Montego Bay request-to-book Worker (CAMBHIGH + CAMBROSE).
 *
 ...
 */
import { montegoBayBookingCore } from "../../../shared/destinations/montego-bay";
...
async function handlePreviewRequest(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message:
          "Live Stripe is not enabled for this Montego Bay Worker. Charge-then-refund still requires an explicit production configuration. No payment was taken.",
      },
      503,
    );
  }
  ...
  const product = findMontegoBayBookingProduct(body.productId);
  ...
  const reference = createBookingReference(montegoBayBookingCore);
  ...
  const destinationBrand = destinationBrandFromCore(montegoBayBookingCore);
